...
for beta1 in np.arange(np.deg2rad(80),np.pi/2, 0.001):      
    #Se definen parametros onda incidente
    M1n=M1*np.sin(beta1)
    gamma1=Gamma(T1)
    ...
    if M1>5:
        T2_initial_guess = Tt_s*0.99901       #M6-->0.98   M7-->0.989
    
    else:
        T2_initial_guess = T1*((-1+7*M1n**2)*(5+M1n**2))/(36*M1n**2)*0.96
        
    T2_s=fsolve(func_T2, T2_initial_guess)

#T2_initial_guess = T1*((-1+7*M1n**2)*(5+M1n**2))/(36*M1n**2)*0.95
    ...
    b=np.append(b,beta1)
    Theta_s=np.append(Theta_s,Theta_deg)
    
    p21_s= np.append(p21_s,p21)
    

#Contrucción de la rama positiva y negatuva onda incidente
b_p =np.linspace(np.deg2rad(80),np.pi/2, 100)

#Gases calorificamente perfectos
P_p_perfect, Theta_d_p_perfect= funShock_perfect(M1,b_p)

#Plot
plt.plot(Theta_d_p_perfect, P_p_perfect, color='red',linestyle='--')
plt.plot(Theta_s, p21_s, color='blue')
